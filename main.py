...
from model import HeterGCL, GB_prop
...
parser.add_argument('-ta', '--str_aug', type=str, default='learned',
                    choices=['para', 'knn', 'none', 'drop_edge', 'GB'])
...
def train(model, graph_learner, optimizer, data, args):
    ...
    elif args.str_aug == 'GB':
        gb_prop = GB_prop(init_GB_num=int(num_nodes ** (1 / 2)), labels = labels_forGB)
        hs1, _ = gb_prop(h1, edge_index)
    else:
